...
fn cost_size_distribution(layout: &LayoutResult, photos: &[Photo]) -> f64 {
...
fn cost_reading_order(layout: &LayoutResult, _photos: &[Photo]) -> f64 {
    ...
    let mut items: Vec<(u16, f64)> = layout.placements
        .iter()
        .map(|p| {
